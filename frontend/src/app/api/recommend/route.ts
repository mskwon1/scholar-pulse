...
export async function POST(req: Request) {
  try {
    ...
    if (!topicDescription) {
      return NextResponse.json({ error: 'topicDescription is required' }, { status: 400 });
    }
...
Return ONLY a valid JSON array of 10 string items, and absolutely no markdown or surrounding text.`;
    ...
    if (!resultText) {
      return NextResponse.json({ error: 'Failed to generate keywords from AI response' }, { status: 500 });
    }
